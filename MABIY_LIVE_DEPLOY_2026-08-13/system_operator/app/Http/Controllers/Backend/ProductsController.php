...
class ProductsController extends Controller
{
    ...
    public function changePaymentStatus(Request $request)
    {
		if (is_null($this->user) || !$this->user->can('order.edit')) {
			abort(403, 'You do not have permission to change order status.');
		}

        $order = DB::table('orders')->where('payment_id', $request->transaction_id)->first();

        return $this->changeOrderStatus($order, $request->status_id, $request->reason);
    }
    ...
    public function bulkAcceptOrders(Request $request)
    {
        ...
        foreach ($orderIds as $orderId) {
            ...
            $orderReference = $order
                ? 'MS' . date('y', strtotime($order->created_at)) . $order->id
                : 'Order ' . $orderId;

            if (!$order) {
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'failed', 'Order was not found.');
                continue;
            }
            if ((int) $order->is_deleted !== 0) {
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'skipped', 'Deleted orders cannot be accepted.');
                continue;
            }
            if ($this->user->hasRole('seller') && !OrderDetails::where('order_id', $orderId)->where('seller_id', $this->user->id)->exists()) {
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'failed', 'You do not have permission to process this order.');
                continue;
            }
            if ((int) $order->status !== 1) {
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'skipped', 'Order is already Accepted or is no longer Pending.');
                continue;
            }

            try {
                $response = $this->changeOrderStatus($order, 2, 'Bulk Accepted');
            } catch (\Throwable $exception) {
                $courierStates = DB::table('pathao_courier_orders')->where('order_id', $orderId)->pluck('state')->all();
                $requiresReview = in_array('uncertain', $courierStates, true) || in_array('in_progress', $courierStates, true);
                $results[] = $this->bulkAcceptResult(
                    $orderId,
                    $orderReference,
                    $requiresReview ? 'uncertain' : 'failed',
                    $requiresReview
                        ? 'Manual Pathao reconciliation required. Processing did not complete safely.'
            ...
            if ($response === 1) {
                DB::table('orders')
                    ->where('id', $orderId)
                    ->where('status', 2)
                    ->update(['order_seen_status' => 1]);
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'accepted', 'Accepted successfully.');
                continue;
            }

            $message = is_array($response) && isset($response['message'])
                ? $response['message']
                : 'The order could not be accepted. Review its address and order details.';
            $courierStates = DB::table('pathao_courier_orders')->where('order_id', $orderId)->pluck('state')->all();
            if (in_array('uncertain', $courierStates, true) || in_array('in_progress', $courierStates, true)) {
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'uncertain', 'Manual Pathao reconciliation required. ' . $message);
            } else {
                $results[] = $this->bulkAcceptResult($orderId, $orderReference, 'failed', $message);
            }
        }

        $summary = ['processed' => count($results), 'accepted' => 0, 'failed' => 0, 'uncertain' => 0, 'skipped' => 0];
        foreach ($results as $result) {
            $summary[$result['result']]++;
        }

        return response()->json(['status' => 1, 'summary' => $summary, 'results' => $results]);
    }

    protected function bulkAcceptResult($orderId, $orderReference, $result, $message)
    {
        return [
    ...
    protected function makePathaoOrder(array $arguments)
    {
        return Helper::pathaoMakeOrder(...$arguments);
    }

    protected function changeOrderStatus($order, $status_id, $reason = null)
    {
        ...
        if ($transaction_id &&  $status_id) {
            if ($order) {
				if ((int) $status_id === 2 && (int) $order->status !== 1) {
					$message = (int) $order->status >= 2
						? 'This order is already Accepted or beyond courier creation.'
						: 'Courier creation is not valid for the current order status.';
                ...
                $address = null;
				$isPickpointOrder = (int) $order->is_pickpoint === 1;

                if ($status_id == 2) {
					if ($isPickpointOrder) {
						$pickpoint = Pickpoints::where('id', $order->address_id)->where('status', 1)->first();
						if (!$pickpoint) {
							return 0;
						}
					} else {
					$address = Addresses::where('id', $order->address_id)
						->where('user_id', $order->user_id)
						->where('is_deleted', 0)
						->first();
                    if (!$address || trim((string) $address->shipping_address) === '') {
                        return 0;
                    }

                    $district = $address->district;
                    $upazila = $address->upazila;
                    $isUnionFreeNormalAddress = $address->shipping_district !== null
                        && $address->shipping_thana !== null
                        && $address->shipping_union === null;
                    $hasValidDistrictAndUpazila = $district
                        && $upazila
                        && (int) $upazila->district_id === (int) $district->id;

                    if ($isUnionFreeNormalAddress) {
                        if (!$hasValidDistrictAndUpazila) {
                            return 0;
                        }
                    } else {
                        $union = $address->union;
                        $hasValidMappedLocation = $hasValidDistrictAndUpazila
                            && $union
                            && (int) $union->upazila_id === (int) $upazila->id
                            && is_numeric($district->city_id)
                            && (int) $district->city_id > 0
                            && is_numeric($upazila->zone_id)
                            && (int) $upazila->zone_id > 0
                            && is_numeric($union->area_id)
                            && (int) $union->area_id > 0;
                ...
                if ($status_id == 2 || $status_id == 6) {
					...
					if($status_id == 2 && !$isPickpointOrder){
                        ...
                        foreach($SellerWiseGroup as $group){
							...
							$merchant_order_id = 'MB'. date('y', strtotime($order->created_at)) .$order->id;
							if (count($SellerWiseGroup) > 1) {
								$merchant_order_id .= '-S' . $group['shop_info']['seller_id'];
							}

							$courier = DB::table('pathao_courier_orders')
								->where('order_id', $order->id)
								->where('seller_id', $group['shop_info']['seller_id'])
								->first();
							$ownsCourierAttempt = false;
							if (!$courier) {
								$ownsCourierAttempt = DB::table('pathao_courier_orders')->insertOrIgnore([
									'order_id' => $order->id,
									'seller_id' => $group['shop_info']['seller_id'],
							...
							if ($courier && $courier->state === 'confirmed') {
								continue;
							}
							if ($courier && $courier->state === 'uncertain') {
								DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
								return ['status' => 0, 'message' => 'Pathao courier outcome is uncertain. Check the Pathao Merchant Panel and reconcile it before retrying.'];
							}
							if ($courier && $courier->state === 'retryable') {
								$ownsCourierAttempt = DB::table('pathao_courier_orders')
									->where('id', $courier->id)
									->where('state', 'retryable')
									->update(['state' => 'in_progress', 'last_error' => null, 'updated_at' => now()]) === 1;
							}
							if (!$ownsCourierAttempt) {
								if ($courier && $courier->state === 'in_progress' && strtotime($courier->updated_at) <= now()->subMinutes(15)->timestamp) {
									DB::table('pathao_courier_orders')
										->where('id', $courier->id)
										->where('state', 'in_progress')
										->update(['state' => 'uncertain', 'last_error' => 'Courier attempt did not complete locally.', 'updated_at' => now()]);
									DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
									return ['status' => 0, 'message' => 'A previous Pathao courier attempt did not complete. Check the Pathao Merchant Panel and reconcile it before retrying.'];
								}
								DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
								return ['status' => 0, 'message' => 'Pathao courier creation is already in progress. Please wait and do not submit again.'];
							}
                            ...
                            $recipient_zone = null;
                            $recipient_area = null;
                            if ($address->shipping_union !== null) {
                            ...
                            $response = $this->makePathaoOrder([$store_id, $merchant_order_id, $recipient_name, $recipient_phone, $recipient_address, $recipient_city, $recipient_zone, $recipient_area, $delivery_type, $item_type, $special_instruction, $item_quantity, $item_weight, $amount_to_collect, $item_description]);
							$outcome = is_object($response) ? ($response->_local_outcome ?? 'uncertain') : 'uncertain';
							if ($outcome === 'confirmed') {
								$confirmed = DB::table('pathao_courier_orders')
									->where('order_id', $order->id)
									->where('seller_id', $group['shop_info']['seller_id'])
									->where('state', 'in_progress')
									->update([
										'state' => 'confirmed',
										'consignment_id' => $response->_local_consignment_id,
										'last_error' => null,
										'updated_at' => now(),
									]);
								if ($confirmed !== 1) {
									DB::table('pathao_courier_orders')
										->where('order_id', $order->id)
										->where('seller_id', $group['shop_info']['seller_id'])
										->update(['state' => 'uncertain', 'last_error' => 'Pathao success could not be persisted safely.', 'updated_at' => now()]);
									DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
									return ['status' => 0, 'message' => 'Pathao courier was returned but could not be confirmed locally. Manual reconciliation is required.'];
								}
							} elseif ($outcome === 'pre_network_failure') {
								DB::table('pathao_courier_orders')
									->where('order_id', $order->id)
									->where('seller_id', $group['shop_info']['seller_id'])
									->where('state', 'in_progress')
									->update(['state' => 'retryable', 'last_error' => 'Pathao connection runtime is unavailable.', 'updated_at' => now()]);
								DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
								return ['status' => 0, 'message' => 'Pathao connection is unavailable on this server. Please contact the administrator.'];
							} elseif ($outcome === 'rejected') {
								DB::table('pathao_courier_orders')
									->where('order_id', $order->id)
									->where('seller_id', $group['shop_info']['seller_id'])
									->where('state', 'in_progress')
									->update(['state' => 'retryable', 'last_error' => 'Pathao rejected the courier request.', 'updated_at' => now()]);
								DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
								return ['status' => 0, 'message' => 'Pathao rejected the courier request. Review the address and retry when corrected.'];
							} else {
								DB::table('pathao_courier_orders')
									->where('order_id', $order->id)
									->where('seller_id', $group['shop_info']['seller_id'])
									->where('state', 'in_progress')
									->update(['state' => 'uncertain', 'last_error' => 'Pathao response was not safely confirmed.', 'updated_at' => now()]);
								DB::table('seller_account_history')->where('order_id', $order->id)->where('status', 1)->update(['status' => 0]);
								return ['status' => 0, 'message' => 'Pathao courier outcome is uncertain. Check the Pathao Merchant Panel and reconcile it before retrying.'];
							}
                ...
                if ($order->payment_method == 'cash_on_delivery' && $status_id == 6) {
