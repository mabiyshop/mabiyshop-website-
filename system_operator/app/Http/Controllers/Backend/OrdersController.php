...
class OrdersController extends Controller
{
    ...
    public function getOrderList($start_date, $end_date)
    {
        ...
        return Datatables::of($data)->addIndexColumn()
            ...
            ->addColumn('bulk_select', function ($row) {
                if ((int) $row->status === 1 && (int) $row->is_deleted === 0 && Auth::user()->can('order.edit')) {
                    return '<input type="checkbox" class="bulk-order-checkbox" value="' . (int) $row->id . '" aria-label="Select order ' . (int) $row->id . '">';
                }
            ...
            ->rawColumns(['bulk_select', 'created_at', 'statistics', 'user_id', 'shipping_name', 'shipping_phone', 'product_qty', 'paid_amount', 'total_amount', 'status', 'action'])->make(true);
    }
    ...
    public function getSoldInvoices()
    {
        ...
        return Datatables::of($data)->addIndexColumn()
            ...
            ->addColumn('action', function ($row) {
				...
				$btn = $btn . '<a class="btn btn-sm btn-danger" target="_blank" href="'.route('admin.order.sold.invoice.print', ['courier', $row->id]).'">Courier</a>';
    ...
    public function printInvoice($type, $id){
        ...
        if ($type == 'pos') {
			...
			return view('backend.pages.order.invoice.courier', compact('order'));
		}
    }
