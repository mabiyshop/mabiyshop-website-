            ...
            foreach ($products as $key => $value) {
                ...
                if($value['product_type'] == '' || $value['title'] == ''){
                    $notImported[] = 'Row ID- '.$i.' : Category - '.$value['title'].' could not imported.';
                        ...
                        $product->special_price_start = (isset($value['special_price_start']) && $value['special_price_start']) ? date('Y-m-d h:i:s', strtotime($value['special_price_start']))  : '';

                        $product->special_price_end = (isset($value['special_price_end']) && $value['special_price_end']) ? date('Y-m-d h:i:s', strtotime($value['special_price_end'])) : '';
        ...
        if($request->hasFile('import_category_file')){
            ...
            foreach ($categories as $key => $value) {
                ...
                if($value['title'] == ''){
                    ...
                    $parent_category = Category::where('title', $value['parent_category_title'])->first();
                    ...
                    if (!$category) {
                        ...
                        $category->slug = $value['slug'] ?? Helper::generateUniqueSlug('category','title',$value['title']);
        ...
        if($request->hasFile('import_brand_file')){
            ...
            foreach ($brands as $key => $value) {
                ...
                if($value['title'] == ''){
                    ...
                    $brand = Brand::where('title', $value['title'])->first();

                    if (!$brand) {
                        $slug = Helper::generateUniqueSlug('brand','title',$value['title']);

                        $brand = new Brand();
                        $brand->title = $value['title'];
                        $brand->slug = ($value['slug'] == '') ? Helper::generateUniqueSlug('brand','title',$value['title']) : $value['slug'];
            ...
            foreach ($sellers as $key => $value) {
                ...
                if($value['seller_name'] == '' || $value['seller_email'] == '' || $value['shop_name'] == '' || $value['shop_email'] == ''){
                    ...
                    if(Admins::where('email', $value['seller_email'])->exists()){
                        ...
                        $slug = Helper::generateUniqueSlug('seller','title',$value['seller_name']);
                        ...
                        $seller->email = $value['seller_email'];
                        $seller->password = ($value['password'] == '') ? $value['password'] : Hash::make($value['phone']);
                        $seller->phone = ($value['phone']) ? '0'.$value['phone'] : null;
                        $seller->assignRole(['seller']);
                        $seller->status = isset($value['status']) ? 1 : 0;
                        $seller->save();

                        $division = Division::where('title', $value['division'])->first();
                        $district = District::where('title', $value['district'])->first();
                        $area = Upazila::where('title', $value['area'])->first();
                        $shop_union = Union::where('title', $value['shop_union'])->first();

                        $shopinfo = new ShopInfo();
                        $shopinfo->name = $value['shop_name'];
                        $shopinfo->seller_id = $seller->id;
                        $shopinfo->slug = ($value['slug'] == '') ? Helper::generateUniqueSlug('seller','title',$value['seller_name']) : $value['slug'];
                    ...
                    if(!$user_email && !$user_phone){
                        ...
                        try{
                            ...
                            if($customer){
                                ...
                                $address->shipping_first_name = $customer->name ?? null;
                                $address->shipping_address = ($value['address']) ? $value['address'] : null;
                                $address->shipping_phone = $customer->phone ?? null;
                                $address->shipping_email = $customer->email ?? null;
                                try{
                                    $address->save();
                                    ...
                                    $notImported[] = 'Row ID '.$i.' was not imported. Error: '.$address;
                                }
                            }
