...
{
    private const COURIERS = ['pathao', 'steadfast', 'paperfly', 'redx'];
    ...
    public function getHistory(string $phone, array $options = []): array
    {
        ...
        try {
            ...
            $summaries = $payload['data']['Summaries'] ?? null;

            if (!is_array($summaries)) {
                return $this->emptyResult($normalizedPhone, 'unavailable', 'malformed_provider_response');
            }

            return $this->normalizeResponse($normalizedPhone, $summaries);
    ...
    private function normalizeResponse(string $phone, array $summaries): array
    {
        $indexedSummaries = [];

        foreach ($summaries as $name => $summary) {
            $indexedSummaries[strtolower((string) $name)] = $summary;
        }
        ...
        foreach (self::COURIERS as $courierName) {
            $summary = $indexedSummaries[$courierName] ?? null;
            $courier = $this->normalizeCourier($summary);
            $couriers[$courierName] = $courier;

            if (
                $courier['available']
                && $courier['data_type'] === 'delivery'
                && $courier['total'] !== null
    ...
    private function normalizeCourier($summary): array
    {
        ...
        $total = $this->integerValue($summary['total'] ?? null);
        $successful = $this->integerValue($summary['success'] ?? null);
        $failed = $this->integerValue($summary['cancel'] ?? null);
