...
@section('title', 'Supplier Pay - ' . config('concave.cnf_appname'))
@section('content')
    <div class="grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <span class="card-title">Dashboard > Accounting > Supplier Pay</span>
                <div class="row">
                    <div class="col-md-10">
                    ...
                    <div class="col-md-2">
                    	<a class="btn btn-success float-right" href="" data-toggle="modal" data-target="#manage_due_payment_modal" >New Pay</a>
                    </div>
                </div>
            </div>
        </div>
	    ...
	    <div class="modal-dialog modal-md" role="document">
	        <div class="modal-content">
	            <div class="modal-header">
	                <h5 class="modal-title" id="exampleModalLongTitle">Manage Supplier Payment</h5>
	                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	                    <span aria-hidden="true">&times;</span>
	                </button>
	            </div>
	            <div class="modal-body" >
	                <form action="{{ route('admin.accounts.supplier.due.store') }}" class="row" method="POST">
	                    @csrf
	                    @if (Auth::user()->getRoleNames() != '["seller"]')
                            <div class="col-sm-12">
                        ...
                        <div class="col-lg-12">
	                        <div class="form-group">
	                            ...
	                            <select class="selectpicker form-select form-control " name="supplier_id" id="supplier_id" data-show-subtext="true" data-live-search="true" required>
                        ...
                        <div class="col-lg-12">
	                        <div class="form-group">
	                            <label for="">Supplier Payable</label>
	                            <input type="text" name="due" id="supplier_due" class="form-control" placeholder="0.00" readonly>
	                        </div>
	                    </div>
    ...
    <script type="text/javascript">
        function supplierPayment(branch_id = '') {
            var table = jQuery('#dataTable').DataTable({
                ...
                autoWidth: true,
                ajax: {
                    url: "{{ route('admin.accounts.supplier.due.data') }}",
                    type: 'GET',
                    data:{
                    	'branch_id' : branch_id,
                    },
        ...
        function calculateCurrentBalance(){
            let customer_balance = jQuery('#supplier_due').val();
        ...
        jQuery(document).on('change', '#supplier_id', function(e) {
            e.preventDefault();
            let supplier_id = jQuery('#supplier_id').val();
            $.ajax({
                url: "{{  url('/admin/accounts/supplier/history/') }}/"+supplier_id,
                type: "GET",
                dataType: "json",
                success: function (response) {
                    jQuery('#manage_due_payment_modal #supplier_due').val(response.balance);
                    jQuery.each( response.invoice, function( key, value ) {
                        jQuery('#manage_due_payment_modal #invoice_number').append(
                            '<option value="'+value.reference_no+'">'+value.reference_no+'</option>'
                        );
                    });
