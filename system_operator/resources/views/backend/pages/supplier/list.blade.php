...
@section('title', 'Supplier List - ' . config('concave.cnf_appname'))
@section('content')
    <div class="grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <span class="card-title">Dashboard > Supplier</span>
                <a class="btn btn-success float-right" href="{{ route('admin.supplier.create') }}">Create New Supplier</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid-margin stretch-card">
        <form action="{{ route('admin.supplier.bulk.action') }}" method="POST" style="width: 100%;">
            <div class="card">
                <div class="toolbar">
                    <div class="row">
                        <div class="col-md-12">
    ...
    <div class="modal fade" id="supplier_quick_view_modal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">

                <!-- Modal Header -->
                <div class="modal-header">
                    <h5 class="modal-title" id="">Supplier Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body supplier_form_element">
...
@push('footer')
    <script type="text/javascript">
        var table = jQuery('#dataTable').DataTable({
            ...
            autoWidth: true,
            ajax: {
                url: "{{ route('admin.get.supplier.list') }}",
                type: 'GET',
            },
            "order": [
                [0, 'desc']
            ...
            columns: [{
                ...
                },
                {
                    data: 'name',
                    search: {
                        // Set the search value to the concatenated column value
                        value: function(data, _, __) {
        ...
        // Quick View customer
        jQuery(document).on('click', '.supplier_quick_view_btn', function(e) {
            e.preventDefault();
            jQuery.ajax({
                url: "/admin/suppliers/view/" + jQuery(this).attr('data-id'),
                type: "get",
                data: {},
                success: function(response) {
                    jQuery('.supplier_form_element').html(response);
                    jQuery('#supplier_quick_view_modal').modal('show');
                }
            });
        });
    </script>
