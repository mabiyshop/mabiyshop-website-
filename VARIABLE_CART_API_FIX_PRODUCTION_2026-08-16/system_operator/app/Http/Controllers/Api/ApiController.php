...
class ApiController extends Controller
{
				...
				if ($product) {
					...
					if ($request->qty > $product->max_cart_qty) {
						...
						if ($already_in_cart) {
							...
							if ($update) {
								$cart['status'] = 1;
								$cart['data'] = array_merge($already_in_cart->toArray(), $data);
			...
			if ($request->product_id) {
				...
				if ($product) {
					...
					if ($request->qty > $product->max_cart_qty) {
						...
						if ($already_in_cart) {
							//Already Added to cart, we just need to update this product
							$qty = $already_in_cart->qty + ($request->qty ?: 1);
	...
	public function categories()
	{
		...
		if ($lang != 'en') {
			...
			foreach ($categories as $item) {
				...
				foreach ($item->categories as $layer1) {
					$categoryIds[] = $layer1->id;
					foreach ($layer1->categories as $layer2) {
						$categoryIds[] = $layer2->id;
						foreach ($layer2->categories as $layer3) {
							$categoryIds[] = $layer3->id;
						}
					}
				}
			}

			$localizations = CategoryLocalization::whereIn('category_id', $categoryIds)
				->where('is_active', 1)
				->where('lang_code', $lang)
				->get()
				->groupBy('category_id');
		}
		$getLocalization = function ($categoryId) use ($localizations) {
			$items = $localizations->get($categoryId);
		...
		foreach ($categories as $item) {
			...
			if ($lang != 'en') {
				...
				$langData = $getLocalization($item->id);
				if ($langData) {
					$item->title = $langData->title ? $langData->title : $item->title;
				}
				...
				foreach ($item->categories as $layer1) {
					...
					$langData = $getLocalization($layer1->id);
					if ($langData) {
						$layer1->title = $langData->title ? $langData->title : $layer1->title;
					}

					foreach ($layer1->categories as $layer2) {
						$langData = $getLocalization($layer2->id);
						if ($langData) {
						...
						foreach ($layer2->categories as $layer3) {
							...
							$langData = $getLocalization($layer3->id);
							if ($langData) {
	...
	public function site_info(){
		...
		$settingKeys = [
			...
			'phone_number',
			'copyright_text',
			'accepted_payment_image',
			'header_offer_banner_status',
			'header_offer_banner_image',
			'header_offer_banner_linktype',
			'header_offer_banner_link',
			'favicon',
			'return_policy',
			'privacy_policy',
			'warranty_policy',
			...
			'welcome_text',
			'social_login',
			'partial_payment_enable',
			'default_shipping_enable',
		];
		$settings = DB::table('settings')->select('key', 'value')->whereIn('key', $settingKeys)->get();
		$getSetting = function ($key) use ($settings) {
			$setting = $settings->firstWhere('key', $key);
			return $setting ? $setting->value : false;
		};

		$data['header_logo']  =  $getSetting('header_logo');
		$data['footer_logo']  = $getSetting('footer_logo');
		$data['phone_number'] = $getSetting('phone_number');
		$data['copyright_text'] = $getSetting('copyright_text');
		$data['accepted_payment_image'] = $getSetting('accepted_payment_image');


		$data['header_offer_banner_status'] = $getSetting('header_offer_banner_status') ?? null;
		$data['header_offer_banner_image'] = $getSetting('header_offer_banner_image') ?? null;
		$data['header_offer_banner_linktype'] = $getSetting('header_offer_banner_linktype') ?? null;
		$data['header_offer_banner_link'] = $getSetting('header_offer_banner_link') ?? null;

		$data['favicon'] = $getSetting('favicon');
		$data['return_policy'] = $getSetting('return_policy');
		$data['privacy_policy'] = $getSetting('privacy_policy');
		$data['warranty_policy'] = $getSetting('warranty_policy');
		$data['terms_of_use'] = $getSetting('terms_of_use');
		$data['facebook'] = $getSetting('social_links_facebook') ?? null;
		$data['twitter'] = $getSetting('social_links_twitter') ?? null;
		$data['instagram'] = $getSetting('social_links_instagram') ?? null;
		$data['youtube'] = $getSetting('social_links_youtube') ?? null;
		$data['pinterest'] = $getSetting('social_links_pinterest') ?? null;
		$data['welcome_text'] = $getSetting('welcome_text');
		$data['social_login'] = $getSetting('social_login');
		...
		$data['partial_payment_enable'] = (int)$getSetting('partial_payment_enable') ?? null;
		...
		if ($getSetting('default_shipping_enable') == 1){
...
// 	}
    ...
    public function getProductBySlug($slug)
    {
        ...
        if (!$product) {
            return response()->json(['status' => 0, 'message' => 'Product Not Found.'], 200);
        }

        // Localization
        $lang = app()->getLocale();
        if ($lang != 'en') {
            $langData = ProductLocalization::where('product_id', $product->id)
                ->where('is_active', 1)
                ->where('lang_code', $lang)
                ->first();
            ...
            if ($langData) {
                ...
                $product->short_description = $langData->short_description ?? $product->short_description;
                $product->description = $langData->description ?? $product->description;
            }
        }
        ...
        if ($user) {
            ...
            $availableShippingOptions = \Helper::get_shipping_cost($user->id, $product->seller_id, $product->id);
        }

        $product->shipping_options = $availableShippingOptions;

        // Calculate price and discounts
        $calculatePriceAfterOffer = function ($loadedProduct) {
            $specialPriceStart = Carbon::createFromFormat('Y-m-d H:i:s', $loadedProduct->special_price_start);
            $specialPriceEnd = Carbon::createFromFormat('Y-m-d H:i:s', $loadedProduct->special_price_end);
            $currentDate = Carbon::now();

            if (($specialPriceStart <= $currentDate) && ($specialPriceEnd >= $currentDate)) {
                if ($loadedProduct->special_price > 0) {
                    if ($loadedProduct->special_price_type == 1) {
                        return $loadedProduct->price - $loadedProduct->special_price;
                    } elseif ($loadedProduct->special_price_type == 2) {
                        $percent = $loadedProduct->special_price;
                        $price = $loadedProduct->price;
                        $discount = $price * ($percent / 100);
                        return $price - $discount;
                    }
                } else {
                    return $loadedProduct->price;
                }
            } else {
                return $loadedProduct->price;
            }
        ...
        $calculateDiscountAvailable = function ($loadedProduct) {
            $specialPriceStart = Carbon::createFromFormat('Y-m-d H:i:s', $loadedProduct->special_price_start);
            $specialPriceEnd = Carbon::createFromFormat('Y-m-d H:i:s', $loadedProduct->special_price_end);
            $currentDate = Carbon::now();

            return ($specialPriceStart <= $currentDate) && ($specialPriceEnd >= $currentDate);
        };

        $calculateDiscountAmount = function ($loadedProduct) {
            $specialPriceStart = Carbon::createFromFormat('Y-m-d H:i:s', $loadedProduct->special_price_start);
            $specialPriceEnd = Carbon::createFromFormat('Y-m-d H:i:s', $loadedProduct->special_price_end);
            $currentDate = Carbon::now();
            ...
            if (($specialPriceStart <= $currentDate) && ($specialPriceEnd >= $currentDate)) {
                if ($loadedProduct->special_price > 0) {
                    if ($loadedProduct->special_price_type == 1) {
                        return $loadedProduct->special_price;
                    } elseif ($loadedProduct->special_price_type == 2) {
                        $percent = $loadedProduct->special_price;
                        $price = $loadedProduct->price;
        ...
        $product->price_after_offer = $calculatePriceAfterOffer($product);
        $product->discount_available = $calculateDiscountAvailable($product);
        $product->discount_amount = $calculateDiscountAmount($product);

        // Add seller information
        if ($product->seller_id) {
            $shop = ShopInfo::select('name', 'slug')->where('seller_id', $product->seller_id)->first();
            $product->shop_name = optional($shop)->name;
            $product->shop_slug = optional($shop)->slug;
            $product->seller_ratings_info = Cache::remember(
                "seller_rating_{$product->seller_id}",
                3600,
                function () use ($product) {
                    return \Helper::sellerRatingCalculation($product->seller_id);
                }
            );
        }
        ...
        if ($product->meta) {
            $metaData = $product->meta->filter(function ($meta) {
                return in_array($meta->meta_key, [
                    'custom_options',
                    'product_shipping_option',
                    'product_miscellaneous_information',
                    'tab_option'
                ]);
            });
            ...
            foreach ($metaData as $meta) {
                ...
                if ($meta->meta_key == 'custom_options') {
                    $quantities = collect($meta->meta_value)
                        ->pluck('value.*.qty')
                        ->flatten()
                        ->toArray();

                    $product->hidden_option = collect($meta->meta_value)
                        ->mapWithKeys(function ($single_op) {
                            return [$single_op['title'] => true];
                        });

                    $product->calculated_in_stock = array_sum($quantities) > 0;
                }
            }
        }
        ...
        if ($category) {
            ...
            if ($parentCategory) {
                $breadcrumb = optional($parentCategory->parentCategory)->title . ' / ' . $parentCategory->title . ' / ' . $category->title;
            } else {
        ...
        // Average rating and shop verification
        $averageRate = $product->comments()
            ->approvedComments()
            ->avg('rate');

        $product->star_rating_average = round((float) $averageRate, 2);
        $product->shop_verified = \Helper::biponi_varyfication($product->seller_id);
	...
	public function getSingleAddress($address_id)
	{
		$user = auth('customer-api')->user();
		if ($user) {
			$address = Addresses::where('id', $address_id)
				->where('user_id', $user->id)
				->where('is_deleted', 0)
				->first();
			if ($address) {
	...
	public function addNewAddress(Request $request)
	{
		...
		$request->merge([
			'shipping_phone' => BangladeshMobile::normalize($request->input('shipping_phone')),
		]);

		$isSimplifiedAddress = $request->has('delivery_area');
		$isResolvedAddress = !$isSimplifiedAddress
			&& $request->filled('shipping_district')
			&& $request->filled('shipping_thana');
		$validationRules = [
			'shipping_first_name' => 'required|min:4',
			'shipping_phone' => BangladeshMobile::validationRules(),
			'shipping_address' => 'required|max:100',
		];

		if ($isSimplifiedAddress) {
			$validationRules['delivery_area'] = 'required|in:inside_dhaka,outside_dhaka';
		} elseif ($isResolvedAddress) {
			$validationRules['shipping_district'] = 'required|numeric';
			$validationRules['shipping_thana'] = 'required|numeric';
			$validationRules['shipping_union'] = 'nullable|numeric';
		} else {
			$validationRules['shipping_district'] = 'required|numeric';
			$validationRules['shipping_thana'] = 'required|numeric';
			$validationRules['shipping_union'] = 'required|numeric';
		}
		...
		$validator = Validator::make(
			...
			[
				...
				'shipping_phone.regex' => 'Enter a valid 11-digit Bangladesh mobile number starting with 01.',
		...
		if ($isResolvedAddress) {
			...
			$upazila = DB::table('upazilas')->where('id', $request->shipping_thana)->first();
			$union = $request->filled('shipping_union')
				? DB::table('unions')->where('id', $request->shipping_union)->first()
				: null;
			$locationErrors = [];

			if (!$district) {
				$locationErrors['shipping_district'][] = 'The selected district is invalid.';
			} elseif (!is_numeric($district->city_id) || (int) $district->city_id <= 0) {
				$locationErrors['shipping_district'][] = 'The selected district does not have a valid Pathao city mapping.';
			}

			if (!$upazila) {
				$locationErrors['shipping_thana'][] = 'The selected Upazila/Thana is invalid.';
			} else {
				if ($district && (int) $upazila->district_id !== (int) $district->id) {
					$locationErrors['shipping_thana'][] = 'The selected Upazila/Thana does not belong to the selected district.';
				}

				if (!is_numeric($upazila->zone_id) || (int) $upazila->zone_id <= 0) {
					$locationErrors['shipping_thana'][] = 'The selected Upazila/Thana does not have a valid Pathao zone mapping.';
				}
			}

			if ($request->filled('shipping_union')) {
				if (!$union) {
					$locationErrors['shipping_union'][] = 'The selected Union/Area is invalid.';
				} elseif ($upazila && (int) $union->upazila_id !== (int) $upazila->id) {
					$locationErrors['shipping_union'][] = 'The selected Union/Area does not belong to the selected Upazila/Thana.';
				}
			}

			if ($locationErrors) {
				$data['status'] = 0;
				$data['message'] = $locationErrors;
				return response()->json($data, 200);
			}
		}
		...
		if ($request->address_id) {
			$address = Addresses::where('id', $request->address_id)
				->where('user_id', $user->id)
				->where('is_deleted', 0)
				->first();
			if (!$address) {
		...
		if ($isSimplifiedAddress && $request->delivery_area === 'inside_dhaka') {
			$insideDistrictId = trim((string) \Helper::getSettings('default_shipping_inside_location'));
			$district = ctype_digit($insideDistrictId)
				? DB::table('districts')->where('id', $insideDistrictId)->first()
				: null;

			if (!$district) {
				$data['status'] = 0;
				$data['message'] = [
					'delivery_area' => ['The configured Inside Dhaka district is invalid.'],
				];
				return response()->json($data, 200);
			}
		} elseif (!$isSimplifiedAddress && !$isResolvedAddress) {
			$district = DB::table('districts')->where('id', $request->shipping_district)->first();
		}
		...
		if ($isSimplifiedAddress) {
			$address->shipping_division = $request->delivery_area === 'inside_dhaka' ? $district->division_id : null;
			$address->shipping_district = $request->delivery_area === 'inside_dhaka' ? $district->id : null;
			$address->shipping_thana = null;
			$address->shipping_union = null;
		} elseif ($isResolvedAddress) {
			$address->shipping_division = $district->division_id;
			$address->shipping_district = $district->id;
			$address->shipping_thana = $upazila->id;
			$address->shipping_union = $union->id ?? null;
		} else {
			$address->shipping_division = $district->division_id;
   ...
   //  			}
	...
	public function AllRating($productId)
	{
		...
		if ($product) {
			...
			$totalComments = $product->totalCommentsCount();
			$data['average'] = $average > 0 ? $average : 0;
			$data['total_rating'] = $totalComments > 0 ? $totalComments : 1;
			$data['total_rate'] = $totalComments > 0 ? $totalComments : 0;
		} else {
			$data['average'] =  0;
			$data['total_rating'] = 1;
			$data['total_rate'] = 0;
		...
		$data['five_star']  = DB::table('comments')->where('commentable_id', $productId)->where('rate', 5)->count();
		$data['five_star_percentage'] = 'width:' . number_format($data['five_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['four_star']  = DB::table('comments')->where('commentable_id', $productId)->where('rate', 4)->count();
		$data['four_star_percentage'] = 'width:' . number_format($data['four_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['three_star']  = DB::table('comments')->where('commentable_id', $productId)->where('rate', 3)->count();
		$data['three_star_percentage'] = 'width:' . number_format($data['three_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['two_star']  = DB::table('comments')->where('commentable_id', $productId)->where('rate', 2)->count();
		$data['two_star_percentage'] = 'width:' . number_format($data['two_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['one_star']  = DB::table('comments')->where('commentable_id', $productId)->where('rate', 1)->count();
	...
	public function getSimillarProduct(Request $request)
	{
		...
		$calculatePriceAfterOffer = function ($product) {
			$specialPriceStart = Carbon::createFromFormat('Y-m-d H:i:s', $product->special_price_start);
			$specialPriceEnd = Carbon::createFromFormat('Y-m-d H:i:s', $product->special_price_end);
			$currentDate = Carbon::now();
			...
			if ($specialPriceStart <= $currentDate && $specialPriceEnd >= $currentDate) {
				if ($product->special_price > 0) {
					if ($product->special_price_type == 1) {
						...
						$discount = $product->price * ($product->special_price / 100);
		...
		$calculateOfferPercentage = function ($product) {
			$specialPriceStart = Carbon::createFromFormat('Y-m-d H:i:s', $product->special_price_start);
			$specialPriceEnd = Carbon::createFromFormat('Y-m-d H:i:s', $product->special_price_end);
			$currentDate = Carbon::now();
			...
			if ($specialPriceStart <= $currentDate && $specialPriceEnd >= $currentDate) {
				if ($product->special_price) {
					if ($product->special_price_type == 1) {
						...
						return number_format($discount / $product->price * 100, 0);
					} elseif ($product->special_price_type == 2) {
		...
		$verificationSettings = collect();

		if ($simillarProducts->isNotEmpty()) {
			$verificationSettings = \App\Models\Settings::whereIn('key', [
				'flagship_ids',
				'flagship_banner',
			])->get()->groupBy('key');
		}

		$getVerificationSetting = function ($key) use ($verificationSettings) {
			$items = $verificationSettings->get($key);
			return $items ? $items->first() : null;
		};

		$flagshipIdsSetting = $getVerificationSetting('flagship_ids');
		$flagshipBannerSetting = $getVerificationSetting('flagship_banner');

		$calculateVerification = function ($sellerId) use ($flagshipIdsSetting, $flagshipBannerSetting) {
			if (!$flagshipIdsSetting) {
				return false;
			}

			$ids = explode(',', $flagshipIdsSetting->value);
			$banner = $flagshipBannerSetting ? $flagshipBannerSetting->value : false;

			return [
				'if_veryfied' => in_array($sellerId, $ids),
				'veryfied_banner' => $banner ?? 'logo.png',
			];
		};

		$productIds = $simillarProducts->pluck('id');
		$approvedRatings = collect();
		$starRatings = collect();

		if ($productIds->isNotEmpty()) {
			$approvedRatings = DB::table('comments')
				->select(
					'commentable_id',
					DB::raw('AVG(rate) as average'),
					DB::raw('COUNT(*) as total')
				)
				->whereIn('commentable_id', $productIds)
				->where('commentable_type', (new Product)->getMorphClass())
				->where('approved', true)
				->groupBy('commentable_id')
				->get()
				->keyBy('commentable_id');

			$starRatings = DB::table('comments')
				->select(
					'commentable_id',
		...
		foreach ($simillarProducts as $item) {
			$priceAfterOffer = $calculatePriceAfterOffer($item);
			$offerPercentage = $calculateOfferPercentage($item);

			$item->price_after_offer = number_format((int)$priceAfterOffer, 0);
			$item->price = number_format((int)$item->price, 0);
			$item->offer_percentage = number_format((int)$offerPercentage, 0);
			$item->shop_verified = $calculateVerification($item->seller_id);

			$approvedRating = $approvedRatings->get($item->id);
			$starRating = $starRatings->get($item->id);
			$average = $approvedRating ? round((float)$approvedRating->average, 2) : 0;
			$totalComments = $approvedRating ? (int)$approvedRating->total : 0;

			$allRating['average'] = $average > 0 ? $average : 0;
			$allRating['total_rating'] = $totalComments > 0 ? $totalComments : 1;
			$allRating['total_rate'] = $totalComments > 0 ? $totalComments : 0;
			$allRating['five_star'] = $starRating ? (int)$starRating->five_star : 0;
			$allRating['five_star_percentage'] = 'width:' . number_format($allRating['five_star'] / $allRating['total_rating'] * 100, 0, '.', '') . '%;';
			$allRating['four_star'] = $starRating ? (int)$starRating->four_star : 0;
			$allRating['four_star_percentage'] = 'width:' . number_format($allRating['four_star'] / $allRating['total_rating'] * 100, 0, '.', '') . '%;';
			$allRating['three_star'] = $starRating ? (int)$starRating->three_star : 0;
			$allRating['three_star_percentage'] = 'width:' . number_format($allRating['three_star'] / $allRating['total_rating'] * 100, 0, '.', '') . '%;';
			$allRating['two_star'] = $starRating ? (int)$starRating->two_star : 0;
			$allRating['two_star_percentage'] = 'width:' . number_format($allRating['two_star'] / $allRating['total_rating'] * 100, 0, '.', '') . '%;';
			$allRating['one_star'] = $starRating ? (int)$starRating->one_star : 0;
			$allRating['one_star_percentage'] = 'width:' . number_format($allRating['one_star'] / $allRating['total_rating'] * 100, 0, '.', '') . '%;';
			$allRating['average_percentage'] = 'width:' . ($allRating['average'] / 5 * 100) . '%;';
			$item->AllRating = $allRating;
		}
	...
	public function updateDefaultAddress(Request $request)
	{
		...
		if ($user && $request->address_id) {
			$isPickpoint = (int) $request->pickpoint === 1;
			$address = $isPickpoint
				? Pickpoints::where('id', $request->address_id)->where('status', 1)->first()
				: Addresses::where('id', $request->address_id)
					->where('user_id', $user->id)
					->where('is_deleted', 0)
					->first();
			...
			DB::table('users')->where('id', $user->id)->update([
				...
				'default_pickpoint_address' => $isPickpoint ? 1 : 0,
			]);
		...
		if ($user || $session_key) {
			...
			if ($user) {
				...
				if ($user->default_pickpoint_address == 1) {
					...
					$default_address = Addresses::where('id', $user->default_address_id)
						->where('user_id', $user->id)
						->where('is_deleted', 0)
						->first();
					$hasPathaoLocationInputs = $default_address
						&& $default_address->shipping_district !== null
						&& $default_address->shipping_thana !== null;
					$pathaoDistrict = $hasPathaoLocationInputs
						? District::find($default_address->shipping_district)
						: null;
					$pathaoUpazila = $hasPathaoLocationInputs
						? Upazila::find($default_address->shipping_thana)
						: null;
					$isPathaoPriceReady = $pathaoDistrict
						&& $pathaoUpazila
						&& (int) $pathaoUpazila->district_id === (int) $pathaoDistrict->id
						&& is_numeric($pathaoDistrict->city_id)
						&& (int) $pathaoDistrict->city_id > 0
						&& is_numeric($pathaoUpazila->zone_id)
						&& (int) $pathaoUpazila->zone_id > 0;

					if ($hasPathaoLocationInputs && !$isPathaoPriceReady) {
						$data['status'] = 0;
						$data['message'] = 'The selected delivery address does not have a valid Pathao location mapping. Please update your address.';
						return response()->json($data, 200);
					}
					$isSimplifiedInsideDhaka = $default_address
						&& in_array($default_address->shipping_district, $default_shipping_inside_location_ids)
						&& $default_address->shipping_thana === null
						&& $default_address->shipping_union === null;
					$isSimplifiedOutsideDhaka = $default_address
						&& $default_address->shipping_district === null
						&& $default_address->shipping_thana === null
						&& $default_address->shipping_union === null;
					$isSimplifiedAddress = $isSimplifiedInsideDhaka || $isSimplifiedOutsideDhaka;

					if ($isSimplifiedAddress || \Helper::getsettings('default_shipping_enable') == 1){

						if ($isSimplifiedInsideDhaka || ($default_address && in_array($default_address->shipping_district, $default_shipping_inside_location_ids))){
					...
					if($pathao_shipping_enable == 1 && $isPathaoPriceReady){
						$shipping_cost = 0;
						$shipping_cost_grocery = 0;

						$recipient_city = $pathaoDistrict->city_id;
						$recipient_zone = $pathaoUpazila->zone_id;
	...
	public function Order(Request $request){
		...
		if ($user) {
			...
			$default_shipping_inside_location = \Helper::getSettings('default_shipping_inside_location');
			$default_shipping_inside_location_ids = explode(',', $default_shipping_inside_location);
			$hasPathaoLocationInputs = $userAddress
				&& $userAddress->shipping_district !== null
				&& $userAddress->shipping_thana !== null;
			$pathaoDistrict = $hasPathaoLocationInputs
				? District::find($userAddress->shipping_district)
				: null;
			$pathaoUpazila = $hasPathaoLocationInputs
				? Upazila::find($userAddress->shipping_thana)
				: null;
			$isPathaoPriceReady = $pathaoDistrict
				&& $pathaoUpazila
				&& (int) $pathaoUpazila->district_id === (int) $pathaoDistrict->id
				...
				&& is_numeric($pathaoUpazila->zone_id)
				&& (int) $pathaoUpazila->zone_id > 0;
			$isSimplifiedInsideDhaka = $userAddress
				&& in_array($userAddress->shipping_district, $default_shipping_inside_location_ids)
				&& $userAddress->shipping_thana === null
				&& $userAddress->shipping_union === null;
			$isSimplifiedOutsideDhaka = $userAddress
				&& $userAddress->shipping_district === null
				&& $userAddress->shipping_thana === null
				&& $userAddress->shipping_union === null;
			$isSimplifiedAddress = $isSimplifiedInsideDhaka || $isSimplifiedOutsideDhaka;
			...
			$total_weight_in_kg = 0;
			$pathao_shipping_enable = \Helper::getSettings('pathao_shipping_enable');
			$default_shipping_enable = \Helper::getSettings('default_shipping_enable');
			...
			foreach ($cartData as $cart) {
				...
				if ($sProduct) {
					$total_weight_in_kg += (\Helper::convertProductWeightToKg($sProduct->id) * $cart->qty);

					if (
						$pathao_shipping_enable != 1
						&& $default_shipping_enable != 1
					...
					if ($user->default_pickpoint_address != 1) {
						...
						if ($sProduct->is_grocery == 'grocery') {
							if ($userAddress->shipping_union === null) {
								$data['status'] = 0;
								$data['message'] = 'Please add a detailed delivery address for grocery delivery.';
								return response()->json($data, 200);
							}
			...
			$SellerWiseGroup = array_values($SellerWiseGroup);
			$useDefaultShipping = $isSimplifiedAddress
				|| ($pathao_shipping_enable != 1 && $default_shipping_enable == 1);

			if ($useDefaultShipping) {
				$useInsideShippingOrigin = $isSimplifiedInsideDhaka
					|| ($userAddress && in_array($userAddress->shipping_district, $default_shipping_inside_location_ids));
				$default_shipping_origin = $useInsideShippingOrigin
					? \Helper::getSettings('default_shipping_inside_origin')
					: \Helper::getSettings('default_shipping_outside_origin');

				if ($total_weight_in_kg > 1) {
					$default_shipping_increase_per_kg = \Helper::getSettings('default_shipping_increase_per_kg');
					$shipping_cost += ($default_shipping_origin + ($total_weight_in_kg * $default_shipping_increase_per_kg));
				} else {
					$shipping_cost += $default_shipping_origin;
				}
			} elseif ($pathao_shipping_enable == 1 && $isPathaoPriceReady) {
				$recipient_city = $pathaoDistrict->city_id;
				$recipient_zone = $pathaoUpazila->zone_id;
				foreach($SellerWiseGroup as $group){
					...
					$calculate_price = \Helper::calculateShippingCost($store_id, $item_type, $delivery_type, $item_weight, $recipient_city, $recipient_zone);
					if (
						!is_object($calculate_price)
						|| ($calculate_price->type ?? null) !== 'success'
						|| !isset($calculate_price->data->price)
						|| !isset($calculate_price->data->discount)
						|| !isset($calculate_price->data->additional_charge)
						|| !isset($calculate_price->data->promo_discount)
					) {
						$data['status'] = 0;
						$data['message'] = 'Unable to calculate delivery charge. Please try again.';
						return response()->json($data, 200);
					}
			...
			if ($collected) {
				...
				if ($voucherData['status'] == 1) {
					$voucherActivationCode = $voucherData['code'];
				}
			}
			...
			if ($request->usedVoucher) {
				...
				if ($voucherData['status'] == 1) {
					...
					$voucherConsumptionCode = $voucherData['code'];
				}
			}
			...
			$orderData['shipping_address_snapshot'] = $user->default_pickpoint_address != 1 && $userAddress
				? json_encode(Order::shippingAddressSnapshot($userAddress), JSON_UNESCAPED_UNICODE)
				: null;
			...
			if ($voucherActivationCode !== null) {
				CollectedVoucher::where('voucher_id', $voucherActivationCode)
					->where('user_id', $user->id)
					->where('status', 0)
					->update(['status' => 1]);
			}
			if ($voucherConsumptionCode !== null) {
				CollectedVoucher::where('voucher_id', $voucherConsumptionCode)
					->where('user_id', $user->id)
					->update(['status' => 2]);
			}
			...
			foreach ($cartData as $single_item) {
				...
				if ($orderDetailsId = DB::table('order_details')->insertGetId($detailsData)) {
					if ($request->payment_method == 'cash_on_delivery') {
						$stockUpdated = \Helper::update_product_quantity($single_item->product_id, $single_item->qty, $single_item->variable_options, 'subtraction');
						if (!$stockUpdated) {
							throw new \RuntimeException('Unable to update customer order stock.');
						}
					}
					...
					throw new \RuntimeException('Unable to insert customer order details.');
				}
			}
		...
		if ($user) {
			...
			if ($order) {
				if ($order->is_pickpoint != 1) {
					$shippingAddress = $order->historical_shipping_address;
				} else {
					$shippingAddress = Pickpoints::where('id', $order->address_id)
	...
	public function getReturnProduct(Request $request)
	{
		...
		if ($order) {
			...
			if ($products) {
				...
				$data['shipping_address'] = $order->historical_shipping_address;
				return response()->json($data, 200);
			} else {
				$data['status'] = 3;
				$data['message'] = 'Product not found.';
					...
					if ($meta->meta_key == 'custom_options' || $meta->meta_key == 'tab_option') {
						...
						if ($meta->meta_key == 'custom_options') {
							...
							foreach ($meta->meta_value as $single_op) {
								if (
									!is_array($single_op) ||
									!isset($single_op['title']) ||
									!isset($single_op['value']) ||
									!is_array($single_op['value'])
								) {
									continue;
								}

								$optionTitle = $single_op['title'];

								foreach ($single_op['value'] as $item) {
									$quantities[] = $item;

									if ($item['qty']) {
										$specificStock[$optionTitle][] = (int) $item['qty'];
									} else {
										$specificStock[$optionTitle][] = 0;
									}
								}
							}
							foreach ($specificStock as $key => $val) {
	...
	public function resolveAddressLocation(Request $request, AddressLocationResolver $resolver)
	{
		$validator = Validator::make($request->all(), [
		...
		$result = $resolver->resolve($request->address);
